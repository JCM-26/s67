	...
	<div class="container py-4">
		<form @submit.prevent="searchCourses" class="mb-4">
			<div class="input-group">
				<input type="text" v-model="courseName" class="from-control" placeholder="Enter course name" required />
				<button class="btn btn-primary" type="submit" :disabled="isLoading">{{ isLoading ? "Searching..." : "Search"}}</button>
			</div>
		</form>
...
<script setup>
	...
	const searchCourses = async() => {
		try {
			...
			console.error(error);
			noyf.error("Something went wrong while searching.");
		} finally {
